  ...
  <div class="tagsContainer">
    <view class="containerIn">
      ...
      <div class="tag_right" v-if="tagsList.length > 1">
        <el-dropdown>
          <el-button type="primary" style="background-color: #28939d">标签选项<el-icons name="ArrowDown" /></el-button>
          <template #dropdown>
            <el-dropdown-menu
              size="small"
              style="
...
export default defineComponent({
  setup() {
    ...
    const setTags = (route: RouteLocationNormalizedLoaded) => {
      const isExist = tagsList.value.some((item: RouteLocationNormalizedLoaded) => item.path === route.fullPath)
      if (!isExist) {
